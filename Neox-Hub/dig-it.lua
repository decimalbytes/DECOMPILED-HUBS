...
local Window = OrionLib:MakeWindow({Name = "NEOX ⭐ HUB | Digit v1.3 | .gg/neoxhub", HidePremium = false, SaveConfig = false, ConfigFolder = "OrionTest"})
...
homeTab:AddParagraph("New Redeem Code","Added new redeem code")
...
SpotsTab:AddButton({
	Name = "Scorpion Spot (Legendary)",
	Callback = function()
        game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame = CFrame.new(1400,8,472)
  	end    
})


SpotsTab:AddButton({
	Name = "Toxic Sewage (Legendary)",
	Callback = function()
        game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame = CFrame.new(333,8,-285)
  	end    
})


SpotsTab:AddButton({
	Name = "Ice Ball (Legendary)",
	Callback = function()
        game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame = CFrame.new(442,13,1646)
  	end    
})

SpotsTab:AddButton({
	Name = "T-rex Skull (Mythic)",
	Callback = function()
        game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame = CFrame.new(59,12,698)
  	end    
})

SpotsTab:AddButton({
	Name = "Goblin (Legendary)",
	Callback = function()
        game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame = CFrame.new(-880,7,366)
  	end    
})


SpotsTab:AddButton({
	Name = "Lunar gifts (Legendary-Mythic)",
	Callback = function()
        game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame = CFrame.new(627,6,373)
  	end    
})
...
local function CheckIfStaff(CheckPlayer)
	local StaffRole
	...
	if game.CreatorType == Enum.CreatorType.Group then
		...
		for _, Name in ipairs(StaffRoleNames) do
	...
	if StaffRole then
		game.Players.LocalPlayer:Kick(
			string.format(
				"The player '%s' was detected to be a staff member, their role is '%s'.\n\nIf you believe this is false, contact the NEOX HUB",
				CheckPlayer.Name, StaffRole
			)
		)
	end
end
...
itemsTab:AddToggle({
	...
	Callback = function(Value)
		if Value then
			for _, CheckPlayer in ipairs(game.Players:GetPlayers()) do
				CheckIfStaff(CheckPlayer)
			end
			
			game.Players.PlayerAdded:Connect(CheckIfStaff)
		end
	end
})
...
local tropica = mapFolder:FindFirstChild("Tropica")
if tropica then
    print("tropica folder found")
    processModels(tropica, {
        "BoatDealer", "Merchant", "Solar Guy", "Groot", "Strawbee", "ewfewfewf",
        "Sunny Rig", "Leafy"  
    })
else
    print("tropica folder not found!")
end
...
ESPTab:AddToggle({ 
    ...
    Callback = function(state)
        ...
        if tropica then
            print("Tropica folder found")
            processModels(tropica, {"BoatDealer", "Merchant", "Solar Guy", "Groot", "Strawbee", "ewfewfewf", "Sunny Rig", "Leafy"}) 
        else
            print("Tropica folder not found!")
        end
...
ShopTab:AddParagraph("Read Please","If it shows nothing in shovel dropdown then it's mean your executor is not supported")
...
local positionLabel = mainTab:AddLabel("Position : Not Selected")
...
mainTab:AddButton({
    ...
    Callback = function()
        ...
        if humanoidRootPart then
            ...
            positionLabel:Set("Position : " .. tostring(math.floor(position.X)) .. " X " ..
                tostring(math.floor(position.Y)) .. " Y " ..
                tostring(math.floor(position.Z)) .. " Z")
        else
            positionLabel:Set("Error: Unable to find player position.")
        end
    end
})
